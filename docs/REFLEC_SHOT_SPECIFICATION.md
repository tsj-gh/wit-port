...
| **Lv.4** | 5×5・折れ 6・1 マス再訪・`pickGrade2OrientedStage`（`relaxBendVisit`）（旧 **Grade 3** 生成。プレイ **Grade 5** に対応） |
...
| 値 | 経路の作り方（要約） |
|----|----------------------|
| **`default`**（未指定時） | 上記どおり `tryConstructGrade3Path`（ゴールは `topCandidates`、スタートは `bottomCandidates` を乱択） |
...
| **`rSecond`** | **R-Second（RS2）**経路生成。`(b0,b1,b2)`（合計 4・全体の中脚に対応する **`b1`** は RS2-1 幾何で制限。仕様テキストでいう区間への折れ分配と同義）を満たす折れ配分を採用したうえで RS0〜RS1 をポリライン接合する。**`goal` は経路生成の最後に定まる**: 区間への折れ分配のうち **出口区間の折れ数 `b2`** に従い、`S2` から **上辺（`r=0`）・左端（`c=0`）・右端（`c=w-1`）** のいずれかに到達するよう、上／左／右ゴール候補プール（従来 `pickRSecondGoalCell` と同型の幾何）へ向けて `tryOrthogonalPolylineRFirst` 系で脚を求め、**成功した辺上終端セル**を `goal` とする（事前に `goal` を乱択しない）。**出口探索中**に次を満たした場合は、**実装では再訪点 `R` の乱択ループ（`outerR`）の先頭に戻る**（盤外・pathable 外、接頭辞経路セルとの重複、接頭辞上の直角折れ頂点への再踏み。「最下段より下」は盤外に出た場合と同義）。`generateBoardLv4Stage` の外側試行では、従来どおり `pickGrade2OrientedStage` 等へ進む。**N=1**。 |
...
**デバッグ UI**: `?devtj=true` かつデバッグ ON・**Grade 5 以上**のとき、パネルに **Default / R-First** を切り替える。**R-First** 選択中はストックを使わず Worker で都度生成する（ストックは `lv4GenMode` なしで補充されるため、混在を避けるため）。
...
| **`lv4GenMode`** | Grade 5（Lv.4）の `generateBoardLv4Stage` で **`rFirst`** 時は `tryConstructGrade3PathRFirstN1`、**`rSecond`** 時は `tryConstructGrade3PathRSecondN1`（`goal` は出口脚成功時に辺上へ確定） | 同上・Grade 5 以上でデバッグパネル **R-First** / **R-Second** 選択時のみ該当値を渡す（Default では未送信＝従来生成） |
