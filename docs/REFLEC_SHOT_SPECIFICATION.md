...
| **`lv4GenMode`** | Grade 5（Lv.4）の `generateBoardLv4Stage` で **`rFirst`** 時は `tryConstructGrade3PathRFirstN1` | 同上・Grade 5 以上でデバッグパネル **R-First** 選択時のみ `rFirst` を渡す（Default では未送信＝従来生成） |
