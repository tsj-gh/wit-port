...
- **バンパー**: 折れ **4** のみ **盤内の直角折れセルのみ**（`placeDiagonalBumpersInterior`）。**`start` / `goal` には置かない**。バンパー個数は **折れ数と常に一致**（4 折れなら **4** 個）。折れ **6** ではこの「端点に置かない」制約を**課さず**、`placeDiagonalBumpers(path, startPad, goalPad)` により **入口／出口で直交折れがあるときは `start` / `goal` にも斜めバンパーを置いてよい**。正解個数は **`totalDiagonalTurnCount`＝内角の数（`countRightAngles`）＋（入口直交折れがあれば 1）＋（出口直交折れがあれば 1）**（`countRightAngles === 6` の試行では通常 **6 または 7** 本）。
...
6. **向き・パッド・バンパー**: `pickGrade2Bend6OrientedStage` で 90° 回転・`normalizeGrade2OppositePadPolyline`・`swapSlashKey` を適用。正解バンパーは **`placeGrade2Bend6Bumpers` → `placeDiagonalBumpers(path, startPad, goalPad)`**（入口・出口の直交折れを含み、**`start` / `goal` にも配置しうる**）。経路の第 1 歩は **`DIR.U` 限定としない**（`startPad→start` が直交かつ `start` で第 1 辺が曲がるときは `start` にバンパー）。採用条件は **`bumpers.size === totalDiagonalTurnCount(path, startPad, goalPad)`**。
...
Grade 2 の主要条件（`startPad→start` が `DIR.U`、`goalPad` が最終進行延長上の盤外、折れ **4** では第 1 歩 `DIR.U`・バンパー数＝折れ数・`start`/`goal` にバンパーなし、折れ **6** ではバンパー数＝`totalDiagonalTurnCount`・端点にバンパー可）をまとめて試す場合:
