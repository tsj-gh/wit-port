...
5. **仮定メモ（斜めの想像）**: `ds > 0` なら `start` に `BACKSLASH`、`ds < 0` なら `SLASH` を仮定し `S1` には逆種を仮定する（実バンパーは `placeDiagonalBumpers`）。**全体の目標直角折れ数 `targetBends`** は通常、内側試行ごとに **6・7・8** を乱択する。**尾の DFS に振る折れ予算は固定で 3 折れ・4 折れと決まっているわけではなく**、`targetBends − countRightAngles(フックの折れ線)`（種類 A では `start…S2`、B では `goal…G2`）である。開発用（`?devtj=true` のデバッグ ON）では、スライダーで **全体目標 6/7/8** を固定して生成に渡せる（UI の 2〜4 は 6〜8 に対応する内部スケール）。
