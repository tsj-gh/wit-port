...
- **経路**: **折れ数 6** の直交経路で、**ちょうど 1 マスだけ が 2 回通る**。そのマスについて、**1 回目は直進通過**（水平または垂直）、**2 回目は 90° 折れ**とし、**2 回目の入射方向（直前マス→当該マス）は 1 回目と異なる**（折れ点に別方向から再訪）。他のマスは 1 回のみ。
...
- **バンパー**: 盤内の直角折れのみ（`placeDiagonalBumpersInterior`）。再訪マスは 1 回目が直進のため **1 セル 1 バンパー**で整合。個数は **6**（折れ数と一致）。
