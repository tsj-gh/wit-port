...
| **`rSecond`** | **R-Second（RS2）**。`(b0,b1,b2)`（`b1` は RS2-1 幾何で制限）の **`b1` は中脚の折れ本数の合計**（`R→S1` と `S1→中脚先`、`手前→P2` と `P2→R` の直交関節を含む）。**`b2` は出口脚の合計**（`R→S2` の関節を含む）。**中脚 S1〜P2** は `max(0,b1-2)`（短い L 系）と **`b1` そのもの**（R-First の `tryOrthogonalPolyline` と同型の長い中脚）の **両方**を別々に `tryOrthogonalPolylineRFirst` で探索する。それぞれについて **`R→S1` で関節折れあり／なし**と **`P2→R` で関節折れあり／なし**の **3 通り**（両方折れ・S1 のみ・P2 のみ）を、`forcedSecond` 候補を **直交歩／直進歩**に分けて順序付けし、経路形状でフィルタして **明示的に列挙**する（「`b1Open` ＝常に S1・P2 両方が頂点」という前提は置かない）。**再訪点 R を中間で踏まない**候補に限る。短・長が両方成立したときは乱数でどちらかを採用（長めをやや優先）。長め側は乱択ゆれを拾うため **複数回**試行する。S2〜goal は **`max(0,b2-1)`** を `tryOrthogonalPolylineRFirst` に渡す。RS0〜RS1 を接合した **接頭辞**を得たあと、**先に**その接頭辞と盤に **0〜3 回の 90° CCW** を試し、**最終盤面**でスタートが下から上向き射出となる向き（接頭辞の初手が `DIR.U`）に固定する。続けて **oddG**＝**射像 `S2'`** から、**`b2`** に従い **上辺・左端・右端**（その座標系での `r=0` / `c=0` / `c=w-1`）へ seg2 をポリライン接合する（事前に `goal` を乱択しない）。接頭辞＋seg2 に対し **`finalizeGrade2OrientedAfterRotation`**（パッド正規化・`placeDiagonalBumpersInterior`）でステージを確定し、**正規化後の `goal` も上／左／右辺に無い候補は棄却**。**経路完成後の `pickGrade2OrientedStage` は呼ばない**。**出口探索中**に接頭辞と seg2 の **ハード違反**（重複・直角頂点再踏み等）があれば **`outerR` 先頭へ戻る**。**N=1**。 |
