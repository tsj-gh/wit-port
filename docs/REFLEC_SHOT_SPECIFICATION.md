...
3. **横ズレ `ds`**: 符号付き・非ゼロの整数をランダムに取る。絶対値は盤幅に収まる範囲（実装では概ね 1〜4）。  
   - **A**: `S1 = (start.c + ds, start.r)`。`ds > 0` なら `start` に `BACKSLASH`、`ds < 0` なら `SLASH` を仮定し、**同様に** `S1` には逆種の斜めを仮定する（実バンパーは後述の共通ルール）。`S2` は `S1` の**真上** 1 マス `(S1.c, S1.r - 1)`。経路の芯は **`start → S1 → S2`** に続き、`S2` から `goal` までは **再訪なし** の直交経路を DFS で探す（試行ごとに目標折れ数を **6・7・8** のいずれかで乱択し、フック部の折れ数を差し引いた **残り回数**で尾を張る）。  
   - **B**: **A** と対称に、まず **`G1 = (goal.c + ds, goal.r)`**、**`G2 = (G1.c, G1.r + 1)`**（ゴール側は上方向）。`goal → G1 → G2` とし、`G2` から **`start` へ**同様の再訪なし DFS で尾を張り、得られた頂点列を **反転**して `start … goal` とする（DFS の終端ターゲットは **`start`**）。
4. **尾のゴール付近と `goalPad`**: 正式な芯経路は **`start → S1 → S2 → … → Q → goal`**（または **B** の反転後の同型）。**`goalPad` は常に `(goal.c, goal.r - 1)`** と決め打ちするため、出口は **`goal → goalPad` が画面 `DIR.U`**（**`goalPad` へは上**）になる。尾では **`Q → goal`**（左／右／下など）の向きに応じた経路を探索し、確定後 **`placeGrade2Bend6Bumpers` → `placeDiagonalBumpers(path, startPad, goalPad)`** により、`Q → goal` と **`goalPad → goal`（`DIR.D`）** がトレースと整合するよう **`goal` 上の斜めバンパー**（および必要な内点）を決める（**`goal` マスにバンパーが無い**整合配置になる場合もありうるが、`goalPad` 位置は不変）。
5. **検証**: `countRightAngles` が **6〜8**、`grade1NoRevisit`（全マス一意訪問）。十型交差セル条件は課さない。
6. **向き・パッド・バンパー**: `pickGrade2Bend6OrientedStage` で 90° 回転・**折れ 6 専用の** `normalizeGrade2OppositePadPolyline`（`goalPad` を真上固定で解釈）・`swapSlashKey` を適用。正解バンパーは **`placeGrade2Bend6Bumpers` → `placeDiagonalBumpers(path, startPad, goalPad)`**。経路の第 1 歩は **`DIR.U` 限定としない**。採用条件は **`bumpers.size === totalDiagonalTurnCount(path, startPad, goalPad)`** かつ **`goalPad` が厳密に盤外**。
