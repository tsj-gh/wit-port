...
/**
 * [Pad延伸チェック]：経路上の r の最小・最大を Ymin/Ymax とみなし、
 * start を r 減少方向（画面上げ）へ、goal を r 増加方向（画面下げ）へ マスずつ延長する。
 * 延長マスが既に経路上にあれば null（呼び出し側でリトライ）。
 */
function tryPadVerticalPathExtent(
  path: CellCoord[],
  w: number,
  h: number,
  pathable: boolean[][]
  ...
  let newPath = pre.length ? [...pre, ...path] : path.map((x) => ({ ...x }));
  const gCell = newPath[newPath.length - 1]!;

  const post: CellCoord[] = [];
  ...
  if (post.length) newPath = [...newPath, ...post];

  let padExtentKind: "start" | "goal" | "both" | undefined;
  if (pre.length && post.length) padExtentKind = "both";
  else if (pre.length) padExtentKind = "start";
  else if (post.length) padExtentKind = "goal";

  const start = { ...newPath[0]! };
  const goal = { ...newPath[newPath.length - 1]! };
  const out: {
    path: CellCoord[];
    start: CellCoord;
    goal: CellCoord;
    padExtentKind?: "start" | "goal" | "both";
  } = { path: newPath, start, goal };
