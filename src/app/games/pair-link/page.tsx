  ...
  return (
    <>
      ...
      <EducationalValueSection summaryLabel="このパズルの知育効果について（専門解説・知育コラム連携）">
        <h3>開発背景</h3>
        <p>
          Pair-Link（ペアリンク／Numberlink）は、同じ数字の端点同士を一方通行の線で結び、盤上のすべてのマスを通り、線同士が交差しないという三つの制約を同時に満たす古典的な論理パズルです。紙面やワークでは線の訂正に時間がかかり、誤りに気づくまでのフィードバックも遅れがちでした。Wispo
        ...
        <p>
          ...
          は、教師が全体を代行するのではなく、子どもが自力で試すために必要最小限の秩序だけを示す技法です。Pair-Link では「線は交差しない」「マスに余白を残さない」というルールが画面に明示され、経路の発見そのものは児童の反復に委ねられます。規則違反は操作としてすぐに返るため、外部からの叱責に頼らない
